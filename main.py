...
# ========================================================
# [FIX 1] BASE_DIR — semua path aset pakai ini
# Wajib agar file gambar/font ditemukan di Android
BASE_DIR = os.path.dirname(os.path.abspath(__file__))

def asset(filename):
    """Helper path agar aset selalu ditemukan di PC maupun Android."""
...
if IS_ANDROID:
    ...
    if private_path:
        ...
        _log = '/data/data/org.test.brickbreaker/files/bb_error.txt'
...
# ========================================================
# [FIX 2] gfxdraw — import aman dengan fallback
# Jika tidak tersedia, pakai pygame.draw.circle biasa
try:
...
# ========================================================
# [FIX 3] Vibrator Android — support API 26+ (VibrationEffect)
vibrator_service = None
VibrationEffect = None
_USE_VIBRATION_EFFECT = False
...
try:
    ...
    # Android API 26+ wajib pakai VibrationEffect, bukan vibrate(long)
    Build = autoclass('android.os.Build$VERSION')
    if Build.SDK_INT >= 26:
        VibrationEffect = autoclass('android.os.VibrationEffect')
        _USE_VIBRATION_EFFECT = True
...
# [FIX 4] pygame.SCALED — flag ini butuh pygame 2.0+, pakai try/except
try:
    if IS_ANDROID:
    ...
    try:
        ...
        print(f"Display error kritis: {e2}")
        screen = pygame.display.set_mode((WIDTH, HEIGHT))
...
# [FIX 5] SAVE_FILE — path aman di Android (sandbox internal)
SAVE_FILE = asset("savegame.json")
if IS_ANDROID:
    try:
        private_path = os.environ.get('ANDROID_PRIVATE', '')
        if private_path:
...
# --- FONT ---
# [FIX 6] Font path pakai asset() agar ditemukan di Android
FONT_FILE = asset("GameFont.ttf")
if os.path.exists(FONT_FILE):
    ...
    FONT_MINI   = pygame.font.SysFont(None, 28)
    FONT_SUB    = pygame.font.SysFont(None, 32)
    FONT_UI     = pygame.font.SysFont(None, 44)
    FONT_TITLE  = pygame.font.SysFont(None, 72)
    FONT_BUTTON = pygame.font.SysFont(None, 36)

# --- LOAD ASSETS ---
# [FIX 7] Semua load gambar pakai asset() agar path benar di Android
...
def render_sharp_text(text, font, color, surface, position, anchor="center"):
    ...
    sharp_surf = pygame.transform.smoothscale(raw_surf, (max(1, w // 2), max(1, h // 2)))
...
# [FIX 8] haptic_vibrate — mendukung Android API 26+ (VibrationEffect)
def haptic_vibrate(duration_ms):
    if VIBRATION_ON and IS_ANDROID and vibrator_service:
        try:
            if _USE_VIBRATION_EFFECT and VibrationEffect:
...
# [FIX 9] draw_aa_circle — fallback jika gfxdraw tidak ada
...
def calculate_aim_points(start_x, start_y, angle, bricks):
    ...
    for _ in range(max_steps):
        ...
        if hit_brick:
            has_bounced = True
        if has_bounced and len(points) > 2:
            break
...
def spawn_bomb_particles(x, y, particles_list):
    for _ in range(35):
        ...
        particles_list.append(Particle(
            x, y,
            random.choice([RED, YELLOW, (255, 120, 0)]),
...
def spawn_laser_particles(y, particles_list):
    for _ in range(50):
        ...
        particles_list.append(Particle(
            ...
            random.choice([-1, 1]) * speed * 2, random.uniform(-0.5, 0.5),
...
class Brick:
    ...
    def draw(self, surface):
        if self.brick_type == "bomb":
            ...
            render_sharp_text(str(self.hp), FONT_SUB, WHITE, surface,
                              (self.rect.centerx, self.rect.centery - 6))
            render_sharp_text("BOMB", FONT_MINI, RED, surface,
                              (self.rect.centerx, self.rect.centery + 12))
            ...
            render_sharp_text(str(self.hp), FONT_SUB, BLACK, surface,
                              (self.rect.centerx, self.rect.centery - 6))
            render_sharp_text("<-->", FONT_MINI, BLACK, surface,
                              (self.rect.centerx, self.rect.centery + 12))
        else:
    ...
    def update(self, bricks, launcher_x, launcher_y, particles_list):
        ...
        if self.returning:
            ...
            if dist <= return_speed:
                self.x = float(launcher_x)
                self.y = float(launcher_y)
                self.active    = False
                self.returning = False
                return True
            else:
...
class DynamicIslandSettings:
    ...
    def draw(self, surface):
        ...
        pygame.draw.rect(island_surf, (10, 15, 30, a if self.is_open else 255),
                         (0, 0, w, h), border_radius=br)
        pygame.draw.rect(island_surf, (70, 85, 120, a if self.is_open else 255),
                         (0, 0, w, h), width=2, border_radius=br)

        if self.is_open and a > 120 and h > 100:
            island_surf.blit(IMG_VIBRATION, (25, 35))
            render_sharp_text("Haptic Vibration", FONT_SUB, WHITE, island_surf,
                              (70, 52), anchor="midleft")
            vib_color = GREEN if VIBRATION_ON else RED
            pygame.draw.rect(island_surf, vib_color, (250, 35, 80, 35), border_radius=8)
            render_sharp_text("ON" if VIBRATION_ON else "OFF", FONT_MINI,
                              WHITE, island_surf, (290, 52))
            render_sharp_text("FPS Lock", FONT_SUB, WHITE, island_surf,
                              (70, 112), anchor="midleft")
...
# ========================================================
# [FIX 10] Helper touch → mouse event converter
# Di Android, pygame.FINGERDOWN/UP/MOTION juga diterima.
# SDL2 sudah map touch ke MOUSEBUTTONDOWN otomatis,
# tapi ini fallback tambahan untuk device tertentu.
def get_finger_pos(event):
    """Konversi koordinat FINGER (0.0–1.0) ke pixel layar."""
...
def main():
    ...
    # Cache background (grid)
    cached_bg = pygame.Surface((WIDTH, HEIGHT))
    cached_bg.fill(BG_COLOR)
    for gx in range(0, WIDTH, 40):
        pygame.draw.line(cached_bg, (32, 44, 76), (gx, 0), (gx, HEIGHT), 1)
    for gy in range(0, HEIGHT, 40):
        pygame.draw.line(cached_bg, (32, 44, 76), (0, gy), (WIDTH, gy), 1)
    ...
    while True:
        ...
        if game_state == STATE_MENU:
            ...
            pygame.draw.rect(screen, SLIDER_BG, border_rect_outer,
                             width=1, border_radius=12)
            border_rect_inner = pygame.Rect(WIDTH//2 - 174, title_center_y - 35, 350, 85)
            pygame.draw.rect(screen, YELLOW, border_rect_inner,
            ...
            render_sharp_text(f"CONTINUE PLAYING (LV {level})", FONT_BUTTON,
                              BG_COLOR, screen, continue_btn.center)

            new_btn = pygame.Rect(WIDTH//2 - 80, HEIGHT - 120, 160, 45)
            pygame.draw.rect(screen, (150, 40, 30),
                             (new_btn.x, new_btn.y + 4,
                              new_btn.width, new_btn.height),
                             border_radius=22)
            pygame.draw.rect(screen, RED, new_btn, border_radius=22)
            render_sharp_text("NEW GAME", FONT_SUB, WHITE, screen, new_btn.center)
            render_sharp_text("Peringatan: Progress akan diriset", FONT_MINI,
                              (120, 130, 160), screen, (WIDTH//2, HEIGHT - 55))
            render_sharp_text("Credit: Dr.Senku°", FONT_MICRO, (120, 130, 160),
                              screen, (WIDTH//9, HEIGHT - 10))

            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    pygame.quit(); sys.exit()

                # [FIX 10] Tangkap FINGERDOWN juga (Android fallback)
                pos = None
                if event.type == pygame.MOUSEBUTTONDOWN:
                    pos = event.pos
                elif event.type == pygame.FINGERDOWN:
                    pos = get_finger_pos(event)

                if pos:
                    if island_ui.handle_click(pos, level):
                        continue
                    if not island_ui.is_open:
                        if home_btn_rect.collidepoint(pos):
                            game_state = STATE_HOME
                            skin_popup_open = False
                            haptic_vibrate(40)
                        elif continue_btn.collidepoint(pos):
                            bricks = spawn_bricks(level)
                            balls  = [Ball(launcher_x, launcher_y) for _ in range(9 + level)]
                            particles.clear()
                            aiming = True; is_dragging = False
        ...
        # ====================================================
            ...
            shadow_rect = pygame.Rect(back_btn.x + 3, back_btn.y + 4,
            ...
            for i, (m_lvl, m_name) in enumerate(ACHIEVEMENT_MILESTONES.items()):
                ...
                render_sharp_text(m_name, FONT_SUB, text_col, screen,
                                  (90, item_y + 14), anchor="midleft")
                render_sharp_text(f"Level {m_lvl}", FONT_MINI, sub_col, screen,
                                  (90, item_y + 36), anchor="midleft")

            # Popup skin
            skin_is_unlocked = False  # default
            if skin_popup_open:
                ...
                if viewing_skin_idx == 0:
                    ...
                    skin_is_unlocked = req_achievement in UNLOCKED_ACHIEVEMENTS
            ...
            for event in pygame.event.get():
                ...
                if pos:
                    if leader_btn_rect.collidepoint(pos):
                    ...
                    if skin_popup_open:
                        ...
                        if back_btn.collidepoint(pos):
                            game_state = STATE_MENU
                            haptic_vibrate(40)
                        elif (skin_open_btn.collidepoint(pos) or
                              pygame.Rect(WIDTH//2 - 40, profile_y, 80, 80).collidepoint(pos)):
        ...
        # ====================================================
            ...
            for event in pygame.event.get():
                ...
                # [FIX 10] Tangkap FINGERDOWN/UP/MOTION
                pos     = None
                pos_up  = None
                pos_mov = None
                ...
                if event.type == pygame.MOUSEBUTTONUP:
                    ...
                    pos_up = get_finger_pos(event)

                if event.type == pygame.MOUSEMOTION:
                    pos_mov = event.pos
                elif event.type == pygame.FINGERMOTION:
                    pos_mov = get_finger_pos(event)
                ...
                if pos:
                    ...
                    if not island_ui.is_open:
                        if game_state == STATE_PLAYING:
                            if btn_pause_rect.collidepoint(pos):
                                game_state = STATE_PAUSED
                                is_dragging = False; fast_forward = False
                            elif not aiming and skip_btn_rect.collidepoint(pos):
                                shooting = False
                                current_ball_idx = len(balls)
                                for ball in balls:
                                    if ball.active: ball.returning = True
                            elif aiming:
                                is_dragging = True
                                slider_x = max(40, min(WIDTH - 40, pos[0]))
                            else:
                                fast_forward = True
                            ...
                            if resume_btn.collidepoint(pos):
                                ...
                                balls  = [Ball(launcher_x, launcher_y) for _ in range(9 + level)]
                                particles.clear()
                                aiming = True; shooting = False
                                fast_forward = False; game_state = STATE_PLAYING
                            elif mainmenu_btn.collidepoint(pos):
                                save_game_data(level); game_state = STATE_MENU

                if pos_up and not island_ui.is_open:
                    if game_state == STATE_PLAYING:
                        if is_dragging:
                ...
                if pos_mov and is_dragging and game_state == STATE_PLAYING and not island_ui.is_open:
                    slider_x = max(40, min(WIDTH - 40, pos_mov[0]))
            ...
            balls_left = (len(balls) if aiming
                          else max(0, len(balls) - current_ball_idx))
        ...
        # ====================================================
            ...
            if len(new_achievements_queue) > 0:
                ...
                render_sharp_text(f"'{new_achievements_queue[-1]}'", FONT_SUB, WHITE,
            ...
            if show_skin_popup:
                ...
                if elapsed < 5000:
                    ...
                    alpha = 255
                    if elapsed > 1200:
                        alpha = max(0, int(255 * (1500 - elapsed) / 300))
        ...
        # ====================================================
            ...
            render_sharp_text("ULANGI LEVEL", FONT_SUB, (0, 0, 0),
                ...
                if pos:
                    ...
                    if not island_ui.is_open:
                        if retry_btn.collidepoint(pos):
                            ...
                            aiming = True; is_dragging = False
                            shooting = False; game_state = STATE_PLAYING
                        elif menu_go_btn.collidepoint(pos):
                            game_state = STATE_MENU
        ...
        if current_fps_setting == "Infinite":
            clock.tick()
        else:
            clock.tick(int(current_fps_setting))
