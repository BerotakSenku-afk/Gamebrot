...
# ============================================================
# ERROR LOGGER - Harus paling atas!
# ============================================================
_possible_paths = [
    '/sdcard/bb_error.txt',
    '/storage/emulated/0/bb_error.txt',
...
try:
    ...
    import pygame
    pygame.init()
    pygame.mixer.quit()  # FIX: mixer sering crash di Android
    print("pygame OK!")
    ...
    if IS_ANDROID:
        ...
        print(f"Android display: {WIDTH}x{HEIGHT}")
    else:
        WIDTH, HEIGHT = 450, 750
        screen = pygame.display.set_mode((WIDTH, HEIGHT))
        print(f"Desktop display: {WIDTH}x{HEIGHT}")
    ...
    print(f"Game level loaded: {current_game_level}")
    ...
    # --------------------------------------------------------
    print("Loading fonts...")
    FONT_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), "GameFont.ttf")
    if os.path.exists(FONT_FILE):
        FONT_MICRO  = pygame.font.Font(FONT_FILE, int(18 * SCALE))
        FONT_MINI   = pygame.font.Font(FONT_FILE, int(28 * SCALE))
        FONT_SUB    = pygame.font.Font(FONT_FILE, int(32 * SCALE))
        FONT_UI     = pygame.font.Font(FONT_FILE, int(44 * SCALE))
        FONT_TITLE  = pygame.font.Font(FONT_FILE, int(70 * SCALE))
        FONT_BUTTON = pygame.font.Font(FONT_FILE, int(34 * SCALE))
        print("Custom font loaded!")
    else:
        # FIX: Ganti SysFont dengan Font(None) - selalu ada di Android
        FONT_MICRO  = pygame.font.Font(None, int(18 * SCALE))
        FONT_MINI   = pygame.font.Font(None, int(28 * SCALE))
        FONT_SUB    = pygame.font.Font(None, int(32 * SCALE))
        FONT_UI     = pygame.font.Font(None, int(44 * SCALE))
        FONT_TITLE  = pygame.font.Font(None, int(70 * SCALE))
        FONT_BUTTON = pygame.font.Font(None, int(34 * SCALE))
        print("Default font loaded!")

    # --------------------------------------------------------
    # FIX: LOAD ASSETS - Pakai absolute path
    # --------------------------------------------------------
    ...
    SKIN_NAMES = ["Default", "pydroball.png", "Player1.png", "Player2.png",
                  "bola.png", "Gemini.png", "Rzzky.png"]
    SKIN_BALLS    = [None]
    SKIN_PROFILES = [None]
    ...
    def calculate_aim_points(start_x, start_y, angle, bricks):
        ...
        for _ in range(max_steps):
            x += dx; y += dy
            if x - radius <= 0:
                x = radius; dx *= -1; points.append((x,y)); has_bounced = True
            elif x + radius >= WIDTH:
                x = WIDTH-radius; dx *= -1; points.append((x,y)); has_bounced = True
            if y - radius <= 60:
                y = 60+radius; dy *= -1; points.append((x,y)); has_bounced = True
            ...
            for brick in bricks:
                if sim_rect.colliderect(brick.rect):
                    ...
                    points.append((x,y)); hit_brick = True; break
    ...
    class Particle:
        def __init__(self, x, y, color, dx, dy, lifetime, size):
            ...
            self.lifetime=lifetime; self.size=size

        def update(self):
            self.x+=self.dx; self.y+=self.dy
            self.lifetime-=1; self.size-=0.1
    ...
    def spawn_bomb_particles(x, y, particles_list):
        for _ in range(35):
            ...
            particles_list.append(Particle(
                x, y, random.choice([RED, YELLOW, (255,120,0)]),
                math.cos(angle)*speed, math.sin(angle)*speed,
                random.randint(20,40), random.uniform(3,6)))

    def spawn_laser_particles(y, particles_list):
        for _ in range(50):
            speed = random.uniform(2, 6)
            particles_list.append(Particle(
                random.randint(0, WIDTH), y,
                random.choice([CYAN, WHITE, BLUE]),
                random.choice([-1,1])*speed*2,
                random.uniform(-0.5, 0.5),
                random.randint(15,30), random.uniform(2,4)))
    ...
    class Brick:
        ...
        def draw(self, surface):
            cy_top = self.rect.centery - 6
            cy_bot = self.rect.centery + 12
            if self.brick_type == "bomb":
                ...
                render_sharp_text(str(self.hp), FONT_SUB, WHITE,
                                  surface, (self.rect.centerx, cy_top))
                render_sharp_text("BOMB", FONT_MINI, RED,
                                  surface, (self.rect.centerx, cy_bot))
                ...
                render_sharp_text(str(self.hp), FONT_SUB, BLACK,
                                  surface, (self.rect.centerx, cy_top))
                render_sharp_text("<-->", FONT_MINI, BLACK,
                                  surface, (self.rect.centerx, cy_bot))
            else:
                pygame.draw.rect(surface, self.color, self.rect, border_radius=4)
                text_color = BLACK if self.color == YELLOW else WHITE
                render_sharp_text(str(self.hp), FONT_SUB, text_color,
                                  surface, self.rect.center)

    def activate_special_brick(dead_brick, bricks_list, particles_list):
        if dead_brick.brick_type == "bomb":
            haptic_vibrate(80)
            spawn_bomb_particles(dead_brick.rect.centerx,
                                 dead_brick.rect.centery, particles_list)
            for b in bricks_list[:]:
                if math.hypot(b.rect.centerx - dead_brick.rect.centerx,
                              b.rect.centery - dead_brick.rect.centery) <= BRICK_SIZE*4:
                    b.hp -= 1000000; b.update_color()
        ...
        def update(self, bricks, launcher_x, launcher_y, particles_list):
            ...
            if self.returning:
                ...
                if dist <= return_speed:
                    ...
                    self.active=False; self.returning=False; return True
            ...
            if self.x - self.radius <= 0:
                self.x=self.radius; self.dx*=-1
            elif self.x + self.radius >= WIDTH:
                self.x=WIDTH-self.radius; self.dx*=-1
            ...
            for brick in bricks[:]:
                if ball_rect.colliderect(brick.rect):
                    self.dx *= -1
                    if self.dx > 0: self.x = brick.rect.right  + self.radius
                    else:           self.x = brick.rect.left   - self.radius
                    self.x = max(self.radius, min(WIDTH-self.radius, self.x))
                    brick.hp-=1; brick.update_color(); haptic_vibrate(25)
                    if brick.hp <= 0:
                        bricks.remove(brick)
                        activate_special_brick(brick, bricks, particles_list)
                    break

            self.y += self.dy
            if self.y - self.radius <= 60:
                self.y=60+self.radius; self.dy*=-1
            ...
            for brick in bricks[:]:
                if ball_rect.colliderect(brick.rect):
                    self.dy *= -1
                    if self.dy > 0: self.y = brick.rect.bottom + self.radius
                    else:           self.y = brick.rect.top    - self.radius
                    brick.hp-=1; brick.update_color(); haptic_vibrate(25)
                    if brick.hp <= 0:
                        bricks.remove(brick)
                        activate_special_brick(brick, bricks, particles_list)
            ...
            if self.active or self.returning:
                if SELECTED_SKIN_IDX == 0:
                    ...
                    if img:
                        ...
                        draw_aa_circle(surface, self.x, self.y,
    ...
    def draw_launcher_skin(surface, x, y):
        if SELECTED_SKIN_IDX == 0 or not SKIN_BALLS[SELECTED_SKIN_IDX]:
            ...
            surface.blit(SKIN_BALLS[SELECTED_SKIN_IDX],
                         (int(x-6), int(y-6)))
    ...
    class DynamicIslandSettings:
        def __init__(self, current_level_ref):
            ...
            self.min_w = 40; self.min_h = 40
            self.max_w = 370; self.max_h = 160
        ...
        def draw(self, surface):
            ...
            factor = ((w - self.min_w) / (self.max_w - self.min_w)
                      if self.max_w != self.min_w else 1.0)
            ...
            if self.is_open and w > 320:
                ...
                self.vib_btn = self.fps_left = self.fps_right = pygame.Rect(0,0,0,0)

            island_surf = pygame.Surface((max(1,w), max(1,h)), pygame.SRCALPHA)
            br = h//2 if h < 60 else 22
            pygame.draw.rect(island_surf,
                             (10,15,30, a if self.is_open else 255),
                             (0,0,w,h), border_radius=br)
            pygame.draw.rect(island_surf,
                             (70,85,120, a if self.is_open else 255),
                             (0,0,w,h), width=2, border_radius=br)

            if self.is_open and a > 120 and h > 100:
                island_surf.blit(IMG_VIBRATION, (25, 35))
                render_sharp_text("Haptic Vibration", FONT_SUB, WHITE,
                                  island_surf, (70, 52), anchor="midleft")
                vib_color = GREEN if VIBRATION_ON else RED
                pygame.draw.rect(island_surf, vib_color,
                                 (250,35,80,35), border_radius=8)
                render_sharp_text("ON" if VIBRATION_ON else "OFF",
                                  FONT_MINI, WHITE, island_surf, (290, 52))
                render_sharp_text("FPS Lock", FONT_SUB, WHITE,
                                  island_surf, (70, 112), anchor="midleft")
                render_sharp_text("<", FONT_SUB, YELLOW, island_surf, (245, 112))
                render_sharp_text(str(FPS_OPTIONS[FPS_INDEX]), FONT_SUB, WHITE,
                                  island_surf, (290, 112))
        ...
        def handle_click(self, pos, current_level):
            ...
            if not self.is_open:
                ...
                return False
            if self.vib_btn.collidepoint(mx, my):
                VIBRATION_ON = not VIBRATION_ON; haptic_vibrate(40); return True
            elif self.fps_left.collidepoint(mx, my):
                FPS_INDEX=(FPS_INDEX-1)%len(FPS_OPTIONS); haptic_vibrate(30); return True
            elif self.fps_right.collidepoint(mx, my):
                FPS_INDEX=(FPS_INDEX+1)%len(FPS_OPTIONS); haptic_vibrate(30); return True
            ...
            if not main_box.collidepoint(mx, my):
                self.is_open = False; haptic_vibrate(40)
                save_game_data(self.level_ref); return True
            return False
    ...
    def main():
        ...
        # FIX: Deklarasi variabel leaderboard di awal
        ...
        while True:
            ...
            if game_state == STATE_MENU:
                ...
                pygame.draw.rect(screen, SLIDER_BG,
                                 pygame.Rect(WIDTH//2-179, title_center_y-40, 360, 95),
                                 width=1, border_radius=12)
                pygame.draw.rect(screen, YELLOW,
                                 pygame.Rect(WIDTH//2-174, title_center_y-35, 350, 85),
                                 width=2, border_radius=15)
                render_sharp_text("BRICK BLASTER", FONT_TITLE, (12,18,35),
                                  screen, (WIDTH//2+3, title_center_y+6))
                render_sharp_text("BRICK BLASTER", FONT_TITLE, WHITE,
                                  screen, (WIDTH//2, title_center_y))
                render_sharp_text("✦ ENDLESS BOUNCE ✦", FONT_SUB, BALL_COLOR,
                                  screen, (WIDTH//2, title_center_y+70))

                continue_btn = pygame.Rect(WIDTH//2-140, HEIGHT//2-15, 280, 65)
                pygame.draw.rect(screen, (30,120,70),
                                 (continue_btn.x, continue_btn.y+6,
                                  continue_btn.width, continue_btn.height),
                                 border_radius=20)
                pygame.draw.rect(screen, GREEN, continue_btn, border_radius=20)
                render_sharp_text(f"CONTINUE PLAYING (LV {level})",
                                  FONT_BUTTON, BG_COLOR, screen, continue_btn.center)

                new_btn = pygame.Rect(WIDTH//2-80, HEIGHT-120, 160, 45)
                pygame.draw.rect(screen, (150,40,30),
                                 (new_btn.x, new_btn.y+4,
                                  new_btn.width, new_btn.height), border_radius=22)
                pygame.draw.rect(screen, RED, new_btn, border_radius=22)
                render_sharp_text("NEW GAME", FONT_SUB, WHITE, screen, new_btn.center)
                render_sharp_text("Peringatan: Progress akan direset",
                                  FONT_MINI, (120,130,160),
                                  screen, (WIDTH//2, HEIGHT-55))
                render_sharp_text("Credit: Dr.Senku°", FONT_MICRO,
                                  (120,130,160), screen, (WIDTH//9, HEIGHT-10))
                    ...
                    if event.type == pygame.MOUSEBUTTONDOWN:
                        ...
                        if not island_ui.is_open:
                            if home_btn_rect.collidepoint(event.pos):
                                game_state=STATE_HOME; skin_popup_open=False
                                haptic_vibrate(40)
                            elif continue_btn.collidepoint(event.pos):
                                bricks=spawn_bricks(level)
                                balls=[Ball(launcher_x,launcher_y)
                                       for _ in range(9+level)]
                                particles.clear(); aiming=True; is_dragging=False
                                shooting=False; fast_forward=False
            ...
            # ---- HOME ----
                ...
                pygame.draw.rect(screen, (12,18,36),
                                 pygame.Rect(back_btn.x+3, back_btn.y+4,
                                             back_btn.width, back_btn.height),
                                 border_radius=12)
                ...
                render_sharp_text("v", FONT_SUB, WHITE, screen,
                                  (skin_open_btn.centerx, skin_open_btn.centery-2))
                ...
                render_sharp_text(f"{unlocked_ach}/{total_ach}", FONT_SUB, WHITE,
                                  screen, (achieve_x, stats_y))
                render_sharp_text("Achievement", FONT_MINI, WHITE,
                                  screen, (achieve_x-12, stats_y+35))
                pygame.draw.circle(screen, WHITE, (achieve_x+50, stats_y+35), 10)
                screen.blit(pygame.transform.smoothscale(IMG_ACHIEVE,(20,20)),
                            (achieve_x+40, stats_y+25))

                leader_btn_rect = pygame.Rect(leader_x-60, stats_y-20, 150, 60)
                render_sharp_text("--", FONT_SUB, WHITE,
                                  screen, (leader_x-15, stats_y))
                render_sharp_text("Leaderboard", FONT_MINI, WHITE,
                                  screen, (leader_x-15, stats_y+35))
                pygame.draw.circle(screen, WHITE, (leader_x+50, stats_y+35), 10)
                screen.blit(pygame.transform.smoothscale(IMG_LEADER,(18,18)),
                            (leader_x+41, stats_y+25))
                ...
                for i, (m_lvl, m_name) in enumerate(ACHIEVEMENT_MILESTONES.items()):
                    ...
                    pygame.draw.rect(screen,
                                     (20,45,30) if is_unlocked else BG_COLOR,
                                     item_rect, border_radius=8)
                    pygame.draw.rect(screen,
                                     GREEN if is_unlocked else SLIDER_BG,
                                     item_rect, width=2, border_radius=8)
                    screen.blit(IMG_ACHIEVE, (45, item_y+9))
                    render_sharp_text(m_name, FONT_SUB,
                                      WHITE if is_unlocked else (100,100,100),
                                      screen, (90, item_y+14), anchor="midleft")
                    render_sharp_text(f"Level {m_lvl}", FONT_MINI,
                                      YELLOW if is_unlocked else (80,80,80),
                                      screen, (90, item_y+36), anchor="midleft")
                ...
                if skin_popup_open:
                    ...
                    render_sharp_text("PILIH SKIN BOLA", FONT_SUB, WHITE,
                                      screen, (WIDTH//2, popup.y+25))
                    ...
                    if viewing_skin_idx == 0:
                        ...
                        req_ach = list(ACHIEVEMENT_MILESTONES.values())[viewing_skin_idx-1]
                        if req_ach in UNLOCKED_ACHIEVEMENTS:
                            skin_is_unlocked = True

                    if skin_is_unlocked:
                        screen.blit(view_prof, (WIDTH//2-40, popup.centery-40))
                    else:
                        vp_dim = view_prof.copy(); vp_dim.set_alpha(60)
                        screen.blit(vp_dim, (WIDTH//2-40, popup.centery-40))

                    skin_name = SKIN_NAMES[viewing_skin_idx].replace(".png","")
                    render_sharp_text(skin_name, FONT_MINI, WHITE,
                                      screen, (WIDTH//2, popup.centery+55))
                    ...
                    if not skin_is_unlocked:
                        ...
                        render_sharp_text("TERKUNCI", FONT_MINI, WHITE,
                                          screen, action_btn.center)
                    elif viewing_skin_idx == SELECTED_SKIN_IDX:
                        pygame.draw.rect(screen, GREEN,  action_btn, border_radius=8)
                        render_sharp_text("DIPILIH", FONT_MINI, BG_COLOR,
                                          screen, action_btn.center)
                    else:
                        pygame.draw.rect(screen, YELLOW, action_btn, border_radius=8)
                        render_sharp_text("PILIH", FONT_MINI, BLACK,
                                          screen, action_btn.center)

                # FIX: Tidak pakai locals() lagi
                if show_leaderboard_soon:
                    overlay = pygame.Surface((WIDTH,HEIGHT), pygame.SRCALPHA)
                    ...
                    render_sharp_text("COMING SOON", FONT_SUB, YELLOW,
                                      screen, popup_rect.center)
                        ...
                        if skin_popup_open:
                            if left_arrow.collidepoint(event.pos):
                                ...
                                if skin_is_unlocked and viewing_skin_idx!=SELECTED_SKIN_IDX:
                                    ...
                                    save_game_data(level); haptic_vibrate(40)
                            elif not popup.collidepoint(event.pos):
                                skin_popup_open=False; haptic_vibrate(30)
                        else:
                            if back_btn.collidepoint(event.pos):
                                game_state=STATE_MENU; haptic_vibrate(40)
                            elif (skin_open_btn.collidepoint(event.pos) or
                                  pygame.Rect(WIDTH//2-40,profile_y,80,80
                                              ).collidepoint(event.pos)):
                                skin_popup_open=True
                                viewing_skin_idx=SELECTED_SKIN_IDX; haptic_vibrate(40)
            ...
            # ---- PLAYING / PAUSED ----
                ...
                if game_state == STATE_PLAYING:
                    for b in bricks:
                        if b.rect.y >= HEIGHT-150-BRICK_SIZE:
                            danger_color=RED; break
                ...
                render_sharp_text(f"Level {level}", FONT_UI, WHITE,
                                  screen, (20,30), anchor="midleft")
                pygame.draw.rect(screen, SLIDER_BG, btn_pause_rect, border_radius=5)
                render_sharp_text("PAUSE", FONT_SUB, WHITE,
                                  screen, btn_pause_rect.center)

                for event in pygame.event.get():
                    if event.type == pygame.QUIT:
                        save_game_data(level); pygame.quit(); sys.exit()
                    if event.type == pygame.MOUSEBUTTONDOWN:
                        if island_ui.handle_click(event.pos, level): continue
                        if not island_ui.is_open:
                            if game_state == STATE_PLAYING:
                                if btn_pause_rect.collidepoint(event.pos):
                                    ...
                                    is_dragging=False; fast_forward=False
                        ...
                        if game_state==STATE_PLAYING and not island_ui.is_open:
                            if is_dragging:
                                is_dragging=False; aiming=False
                                shooting=True; current_ball_idx=0; shoot_delay=0
                            fast_forward=False
                    elif (event.type == pygame.MOUSEMOTION and is_dragging
                          and game_state==STATE_PLAYING and not island_ui.is_open):
                        slider_x=max(40,min(WIDTH-40,mx))
                        ...
                        if shooting:
                            ...
                            if shoot_delay>=3:
                                if current_ball_idx<len(balls):
                                    balls[current_ball_idx].launch(aim_angle)
                                    current_ball_idx+=1
                                shoot_delay=0

                        all_balls_done = all(not b.active for b in balls)
                        for ball in balls:
                            ball.update(bricks,launcher_x,launcher_y,particles)
                            if not ball.active and not shooting:
                                ball.x=launcher_x; ball.y=launcher_y
                        ...
                        if not aiming and not shooting and all_balls_done:
                            if len(bricks)==0:
                                ...
                                for m_lvl,m_name in ACHIEVEMENT_MILESTONES.items():
                                    if (level>=m_lvl and
                                            m_name not in UNLOCKED_ACHIEVEMENTS):
                                ...
                                if new_achievements_queue:
                                    ...
                                    try:
                                        ach_index = milestones_list.index(
                                            new_achievements_queue[-1])
                                        skin_idx = ach_index + 1
                                ...
                                for brick in bricks:
                                    ...
                                    if brick.rect.y >= HEIGHT-150:
                                        game_state=STATE_GAMEOVER
                                aiming=True; slider_x=WIDTH//2; fast_forward=False
                        if shooting and current_ball_idx>=len(balls):
                            shooting=False

                ratio     = (slider_x-40)/(WIDTH-80)
                aim_angle = (-math.pi+0.15)+ratio*(math.pi-0.3)

                if (game_state==STATE_PLAYING and is_dragging
                        and not island_ui.is_open):
                    path_points = calculate_aim_points(
                        launcher_x, launcher_y, aim_angle, bricks)
                    if len(path_points)>1:
                        for i in range(len(path_points)-1):
                            pygame.draw.line(screen, BALL_COLOR,
                                             path_points[i], path_points[i+1], 2)
                        draw_aa_circle(screen,
                ...
                balls_left = (len(balls) if aiming
                              else len(balls)-current_ball_idx)
                render_sharp_text(f"x{balls_left}", FONT_SUB, BALL_COLOR,
                                  screen, (launcher_x, launcher_y+30))
                ...
                if game_state==STATE_PLAYING and fast_forward and not aiming:
                    ...
                    render_sharp_text("2× SPEED", FONT_MINI, BLACK,
                                      screen, speed_popup_rect.center)
                if game_state==STATE_PLAYING and not aiming:
                    pygame.draw.rect(screen, RED, skip_btn_rect, border_radius=12)
                    render_sharp_text("SKIP", FONT_MINI, WHITE,
                                      screen, skip_btn_rect.center)

                if game_state==STATE_PAUSED:
                    overlay = pygame.Surface((WIDTH,HEIGHT), pygame.SRCALPHA)
                    overlay.fill(OVERLAY_COLOR); screen.blit(overlay, (0,0))
                    popup_rect = pygame.Rect(WIDTH//2-130, HEIGHT//2-130, 260, 260)
                    pygame.draw.rect(screen, BG_COLOR,   popup_rect, border_radius=10)
                    pygame.draw.rect(screen, SLIDER_BG,  popup_rect,
                                     width=3, border_radius=10)
                    render_sharp_text("GAME PAUSED", FONT_UI, YELLOW,
                                      screen, (WIDTH//2, HEIGHT//2-95))
                    ...
                    render_sharp_text("RESUME",    FONT_SUB, WHITE,
                                      screen, resume_btn.center)
                    render_sharp_text("RESTART",   FONT_SUB, BLACK,
                                      screen, restart_btn.center)
                    render_sharp_text("MAIN MENU", FONT_SUB, WHITE,
                                      screen, mainmenu_btn.center)

            # ---- CLEAR ----
            elif game_state==STATE_CLEAR:
                render_sharp_text(f"LEVEL {level} CLEAR!", FONT_TITLE, YELLOW,
                                  screen, (WIDTH//2, HEIGHT//3-35))
                if len(new_achievements_queue)>0:
                    render_sharp_text("Achievement Complete", FONT_UI, GREEN,
                                      screen, (WIDTH//2, HEIGHT//3+10))
                    render_sharp_text(f"'{new_achievements_queue[-1]}'",
                                      FONT_SUB, WHITE,
                                      screen, (WIDTH//2, HEIGHT//3+45))
                ...
                if show_skin_popup:
                    ...
                    if elapsed < 5000:
                        ...
                        pygame.draw.rect(popup_surf, (45,55,85),
                                         (0,0,popup_w,popup_h), border_radius=15)
                        pygame.draw.rect(popup_surf, CYAN,
                                         (0,0,popup_w,popup_h), width=2, border_radius=15)
                        if new_skin_img:
                            img = pygame.transform.smoothscale(new_skin_img, (50,50))
                            popup_surf.blit(img, (10,10))
                        render_sharp_text("NEW SKIN",  FONT_MINI, YELLOW,
                                          popup_surf, (70,22), anchor="midleft")
                        render_sharp_text("UNLOCKED!", FONT_MINI, WHITE,
                                          popup_surf, (70,48), anchor="midleft")
                ...
                render_sharp_text("LANJUT",       FONT_SUB, WHITE,
                                  screen, next_btn.center)
                render_sharp_text("KEMBALI MENU", FONT_SUB, WHITE,
                                  screen, menu_btn.center)
                        ...
                        if not island_ui.is_open:
                            if next_btn.collidepoint(event.pos):
                                ...
                                balls=[Ball(launcher_x,launcher_y)
                                       for _ in range(9+level)]
                                particles.clear(); aiming=True; is_dragging=False
                                shooting=False; game_state=STATE_PLAYING
            ...
            # ---- GAMEOVER ----
                ...
                render_sharp_text("GAME OVER", FONT_TITLE, RED,
                ...
                render_sharp_text("ULANGI LEVEL", FONT_SUB, BLACK,
                                  screen, retry_btn.center)
                render_sharp_text("MENU UTAMA",   FONT_SUB, WHITE,
                                  screen, menu_go_btn.center)
                    ...
                    if event.type == pygame.MOUSEBUTTONDOWN:
                        ...
                        if not island_ui.is_open:
                            if retry_btn.collidepoint(event.pos):
                                ...
                                particles.clear(); aiming=True; is_dragging=False
                                shooting=False; game_state=STATE_PLAYING
                            elif menu_go_btn.collidepoint(event.pos):
                                game_state=STATE_MENU
